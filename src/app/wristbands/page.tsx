...
export default async function WristbandsPage() {
  ...
  const products: CatalogItem[] = dbProducts.map((p: any) => ({ id: p.id, name: p.name, desc: p.description, price: `RS. ${p.price}`, originalPrice: p.oldPrice ? `RS. ${p.oldPrice}` : undefined, numericPrice: parseFloat(p.price.replace(/,/g, "")), rating: p.rating, reviews: parseInt(p.reviewsCount, 10) || 0, img: p.mainImage, gallery: [p.mainImage], category: "wristbands", inStock: p.quantity > 0 }));
